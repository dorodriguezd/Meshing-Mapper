%VALIDATE_HEAD_MAPPING_BIGGER_DOMAIN Check the enlarged-domain remapped DAT.
%
% The report is written under result/bigger_sfera and verifies that the
% remapped DAT preserves the enlarged target mesh syntax while adding the
% five head tissue material labels.
...
originalDatFile = fullfile(repoRoot, 'input', 'mesh_bowls_bigger.dat');
remappedDatFile = fullfile(resultDir, 'mesh_bowls_bigger_with_head.dat');
reportFile = fullfile(resultDir, 'mesh_bowls_bigger_with_head_validation.txt');

if exist(remappedDatFile, 'file') ~= 2
    error('validateHeadMappingBiggerDomain:MissingResult', ...
...
if reportId < 0
    error('validateHeadMappingBiggerDomain:ReportOpenFailed', ...
        'Could not write validation report: %s', reportFile);
end
cleanupReport = onCleanup(@() fclose(reportId));

logLine(reportId, 'Bigger sfera head mapping validation');
...
label21Count = labelCount(labels, counts, 21);
logLine(reportId, '');
logLine(reportId, 'Label 21 element count: %d', label21Count);
assert(label21Count == 0, ...
    'Label 21 still appears in the remapped element block.');
...
fid = fopen(datFile, 'r');
if fid < 0
    error('validateHeadMappingBiggerDomain:OpenFailed', ...
...
if size(coordinateData{1}, 1) ~= coordinateCount
    error('validateHeadMappingBiggerDomain:BadCoordinates', ...
...
while remaining > 0
    ...
    if size(elementData, 1) ~= rowsToRead
        error('validateHeadMappingBiggerDomain:BadElements', ...
...
for row = 1:materialCount
    ...
    if numel(rowValues) < 6
        error('validateHeadMappingBiggerDomain:BadMaterialRow', ...
...
error('validateHeadMappingBiggerDomain:MissingSection', ...
...
if isempty(count) || count < 0
    error('validateHeadMappingBiggerDomain:BadCount', ...
