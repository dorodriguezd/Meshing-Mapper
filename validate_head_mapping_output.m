%VALIDATE_HEAD_MAPPING_OUTPUT Check remapped head DAT labels and syntax.
%
% This script verifies that the remapped bowl/head DAT keeps the core DAT
% structure of the original bowl mesh, that label 21 no longer appears in
% the element block, and that the material-properties section is internally
% consistent after adding the five head labels.
...
originalDatFile = fullfile(repoRoot, 'HeadMapping', 'mesh_bowls.dat');
remappedDatFile = fullfile(resultDir, 'mesh_bowls_with_head.dat');
reportFile = fullfile(resultDir, 'mesh_bowls_with_head_validation.txt');

if exist(remappedDatFile, 'file') ~= 2
    error('validateHeadMappingOutput:MissingResult', ...
...
if reportId < 0
    error('validateHeadMappingOutput:ReportOpenFailed', ...
        'Could not write validation report: %s', reportFile);
end
cleanupReport = onCleanup(@() fclose(reportId));

logLine(reportId, 'Head mapping output validation');
...
fid = fopen(datFile, 'r');
if fid < 0
    error('validateHeadMappingOutput:OpenFailed', 'Could not open file: %s', datFile);
end
...
if size(coordinateData{1}, 1) ~= coordinateCount
    error('validateHeadMappingOutput:BadCoordinates', ...
...
while remaining > 0
    ...
    if size(elementData, 1) ~= rowsToRead
        error('validateHeadMappingOutput:BadElements', ...
...
for row = 1:materialCount
    ...
    if numel(rowValues) < 6
        error('validateHeadMappingOutput:BadMaterialRow', ...
...
error('validateHeadMappingOutput:MissingSection', ...
...
if isempty(count) || count < 0
    error('validateHeadMappingOutput:BadCount', ...
