%VISUALIZE_HEAD_MAPPING_BIGGER_DOMAIN Create validation plots for bigger sfera remap.
%
% Run build_head_mapping_bigger_domain first. All PNGs are saved under
% result/bigger_sfera.
...
remappedDatFile = fullfile(resultDir, 'mesh_bowls_bigger_with_head.dat');

if exist(remappedDatFile, 'file') ~= 2
    error('visualizeHeadMappingBiggerDomain:MissingResult', ...
        'Run build_head_mapping_bigger_domain first to create %s.', remappedDatFile);
end
...
plotSelectedLabelOverlay(gca, headMesh, headLabels, headNames, ...
    headColors, overlayAlphas, 'Initial Head Mesh');
savePng(gcf, fullfile(resultDir, 'head_initial_tissues.png'));

figure('Name', 'Bigger sfera remapped head tissues', 'Color', 'w');
plotSelectedLabelOverlay(gca, remappedHeadMesh, remappedHeadLabels, headNames, ...
    headColors, overlayAlphas, 'Remapped Head Tissues, Bigger Sfera Domain');
savePng(gcf, fullfile(resultDir, 'head_remapped_tissues_bigger_sfera.png'));

for tissueIndex = 1:numel(headLabels)
    tissueName = char(headNames(tissueIndex));
    figureName = sprintf('%s tissue: original vs bigger-domain remap', tissueName);
    ...
    plotSingleTissue(ax, headMesh, headLabels(tissueIndex), ...
        ...
        sprintf('Original %s, label %g', tissueName, headLabels(tissueIndex)));
...
fprintf('Reading SKIN/context labels for bigger-domain validation...\n');
skinContextMesh = readSelectedDatLabels(remappedDatFile, [(1:19).'; 25]);
figure('Name', 'Bigger sfera SKIN with labels 1-19 context', 'Color', 'w');
plotSkinWithContext(gca, skinContextMesh);
savePng(gcf, fullfile(resultDir, 'remapped_skin25_with_labels_1_to_19.png'));
...
fid = fopen(datFile, 'r');
if fid < 0
    error('visualizeHeadMappingBiggerDomain:OpenFailed', ...
...
error('visualizeHeadMappingBiggerDomain:MissingSection', ...
...
if isempty(count) || count < 0
    error('visualizeHeadMappingBiggerDomain:BadCount', ...
...
if isempty(activeElements)
    warning('visualizeHeadMappingBiggerDomain:MissingLabel', ...
...
skinElements = mesh.elements(mesh.elementLabels == 25, :);
...
finishAxes(ax, 'Remapped SKIN label 25 with labels 1-19 context');
hold(ax, 'off');
end

function plotElementGroup(ax, nodes, elements, color, alphaValue, displayName)
if isempty(elements)
    warning('visualizeHeadMappingBiggerDomain:EmptyGroup', ...
